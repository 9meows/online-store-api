...
# Список сетей/адресов ЮKassa(актуально на 2025)
...
async def yookassa_webhook(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid notification: {exc}")
    ...
    result = await session.scalars(select(OrderModel).where(OrderModel.id == order_id))
    order = result.first()
    if order is None:
